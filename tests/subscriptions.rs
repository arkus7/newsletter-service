...
#[actix_rt::test]
async fn subscribe_returns_500_when_subscription_data_was_duplicated() {
    let app = test_helpers::spawn_app().await;
    let client = reqwest::Client::new();
    ...
    assert_eq!(
        200,
        response.status().as_u16(),
        "The API failed instead of returning 200 OK when the payload was {}.",
        body
    );
    ...
    assert_eq!(
        ...
        "The API failed instead of returning 500 InternalServerError when the subscription was duplicated ({})",
        body
    );
}
